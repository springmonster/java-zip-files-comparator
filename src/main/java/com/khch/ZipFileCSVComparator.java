...
public class ZipFileCSVComparator {
  ...
  public boolean compareCSVFilesInZip(String targetZipPath, String expectedZipPath,
    ...
    try (ZipFile targetZipFile = new ZipFile(targetZipPath);
      ...
      if (checkMD5IsSame(targetZipFile, expectedZipFile, targetZipFileEntry,
          expectedZipFileEntry)) {
        return true;
      }

      if (compareEachLine(targetZipFile, expectedZipFile, targetZipFileEntry,
          expectedZipFileEntry)) {
        return true;
      }
  ...
  private boolean checkMD5IsSame(ZipFile targetZipFile, ZipFile expectedZipFile,
    ...
    byte[] targetMd5 = MessageDigest.getInstance("MD5")
        .digest(readAllBytes(targetZipFileInputStream));
    byte[] expectedMd5 = MessageDigest.getInstance("MD5")
        .digest(readAllBytes(expectedZipFileInputStream));
  ...
  public boolean compareEachLine(ZipFile targetZipFile, ZipFile expectedZipFile,
    ...
    try {
      ...
      expectedBufferedREader = new BufferedReader(
          new InputStreamReader(expectedZipFileInputStream1));
      ...
      if (targetList.size() != expectedList.size()) {
        System.out.println(
            "Target file row count: " + targetList.size() + " and expected file row count: "
                + expectedList.size());
        return false;
      }
