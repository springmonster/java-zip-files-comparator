...
public class ZipCSVComparator {
  ...
  public boolean compareCSVFilesInZip(String targetZipPath, String baseLineZipPath,
    ...
    try (ZipFile targetZipFile = new ZipFile(targetZipPath);
      ...
      return readAndCompareEachLine(targetZipFile, baseLineZipFile, targetZipFileEntry,
          baselineZipFileEntry);
    } catch (Exception e) {
  ...
  public boolean readAndCompareEachLine(ZipFile targetZipFile, ZipFile baselineZipFile,
    ...
    try {
      ...
      baselineBufferedReader = new BufferedReader(
          new InputStreamReader(baselineZipFileInputStream1));
   ...
   * 比较两个文件内容是否一致
   * <p>
   * 1. baselineMap中的key为每行的内容，value为每行的数量 2. targetList为每行的内容 3.
   * 比较的过程中，如果targetList中的内容在baselineMap中不存在，则返回false 4.
   * 如果targetList中的内容在baselineMap中存在，则将baselineMap中的value减1，如果减1后的value为0，则将该key从baselineMap中删除 5.
   * 比较结束后，如果baselineMap为空，则返回true，否则返回false
   *
