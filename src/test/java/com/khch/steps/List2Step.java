...
public class List2Step {
  ...
  public void compare() {
    ...
    for (String expectedName : expectedNames) {
      boolean b = zipFileCSVComparator.compareCSVFilesInZip(
          this.targetFolder + File.separator + "target-" + expectedName,
          this.expectedFolder + File.separator + "expected-" + expectedName,
          expectedName.replace(".zip", ".xlsx"),
          expectedName.replace(".zip", ".xlsx"));

      StringBuilder sb = new StringBuilder();
      sb.append("target-").append(expectedName).append(" vs ").append("expected-")
          .append(expectedName);
